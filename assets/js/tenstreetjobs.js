function loadXMLDoc() {
  ...
  xmlhttp.open(
    "GET",
    "https://dashboard.tenstreet.com/api/jr/job_requisitions.php?company_id=34969&jr_api_key=postaljobreqs&client_id=177&password=gh2lkb$P5$dz1Rj4$ko$^N",
    true
  );
  xmlhttp.send();
}
function myFunction(xml) {
  ...
  var table =
    "<thead><tr><th>Job Company</th><th>Job Title</th><th>Description</th><th>City</th><th>State</th><th>Job Link</th></tr></thead><tbody>";
  ...
  for (i = 0; i < x.length; i++) {
    table +=
      ...
      "</td><td>" +
      x[i].getElementsByTagName("JobDescription")[0].childNodes[0].nodeValue +
      "</td><td>" +
      x[i].getElementsByTagName("City")[0].childNodes[0].nodeValue +
      "</td><td>" +
      (x[i].getElementsByTagName("State")[0].childNodes.length >= 1
  ...
  console.log(xmlDoc.getElementsByTagName("JobRequisition")[15]);
  table += "</tbody>";
